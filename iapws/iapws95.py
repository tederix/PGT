...
class MEoS(_fase):
    ...
    * T-x, P-x, preferred input pair to specified a point in two phases region
    ...
    def _phir(self, tau, delta):
        ...
        http://www.iapws.org/relguide/IAPWS-95.html
        """
        fir = fird = firdd = firt = firtt = firdt = firddt = 0
        ...
        for n, d, t in zip(nr1, d1, t1):
            ...
            firddt += n*t*d*(d-1)*delta**(d-2)*tau**(t-1)
        ...
        for n, d, g, t, c in zip(nr2, d2, g2, t2, c2):
            ...
            firdt += n*t*delta**(d-1)*tau**(t-1)*(d-g*c*delta**c)*exp(
                -g*delta**c)
            firddt += n*t*exp(-delta**c)*delta**(d-2)*tau**(t-1)*(
        ...
        for n, d, t, a, e, b, g in zip(nr3, d3, t3, a3, e3, b3, g3):
            ...
            firddt += n*tau**t*exp(-a*(delta-e)**2-b*(tau-g)**2) * \
                (t/tau-2*b*(tau-g)) * \
                (-2*a*delta**d + 4*a**2*delta**d*(delta-e)**2 -
                 4*d*a*delta**(d-1)*(delta-e) + d*(d-1)*delta**(d-2))
        ...
        for n, a, b, A, B, C, D, bt in zip(nr4, a4, b4, Ai, Bi, Ci, Di, bt4):
            ...
            Fddt = -2*(4*C*(delta-1)**2-2)*C*D*(tau-1)*F
            ...
            Deltat = -2*((1-tau)*A*((delta-1)**2)**(0.5/bt))
            Deltadt = -(delta-1)*A*2/bt*((delta-1)**2)**(0.5/bt-1)
            Deltaddt = Deltadt/(d-1) - (delta-1)**2*A*4/bt*(0.5/bt-1)*(
                (d-1)**2)**(0.5/bt-2)
            DeltaB1t = (b-1)*Delta**(b-2)*Deltat
            DeltaB2t = (b-2)*Delta**(b-3)*Deltat
            Deltad2t = 2*Deltadt*Deltad
            DeltaBddt = b*(DeltaB1t*Deltadd + Delta**(b-1)*Deltaddt +
                           (b-1)*(DeltaB2t*Deltadd**2+Delta**(b-2)*Deltad2t))
            ...
            firddt += n*(Delta**b*(2*Fdt+delta*Fddt) + DeltaBt*(2*Fd+delta*Fdd)
                         + 2*DeltaBdt*(F+delta*Fd) + 2*DeltaBd*(Ft+delta*Fdt)
                         + DeltaBddt*delta*F+DeltaBdd*delta*Ft)
    ...
    def _virial(self, T):
        ...
        for n, d, t in zip(nr1, d1, t1):
            ...
            Bt += n*t*d*delta**(d-1)*tau**(t-1)
            Ct += n*t*d*(d-1)*delta**(d-2)*tau**(t-1)
        ...
        for n, d, g, t, c in zip(nr2, d2, g2, t2, c2):
            ...
            C += n*exp(-g*delta**c)*(delta**(d-2)*tau**t*(
                (d-g*c*delta**c)*(d-1-g*c*delta**c)-g**2*c**2*delta**c))
            Bt += n*t*delta**(d-1)*tau**(t-1)*(d-g*c*delta**c)*exp(
                -g*delta**c)
            Ct += n*t*exp(-delta**c)*delta**(d-2)*tau**(t-1)*(
                (d-c*delta**c)*(d-1-c*delta**c)-c**2*delta**c)
            ...
            C += n*tau**t*exp(-a*(delta-e)**2-b*(tau-g)**2)*(
                    -2*a*delta**d+4*a**2*delta**d*(
                        delta-e)**2-4*d*a*delta**2*(
                            delta-e)+d*2*delta)
            Bt += n*delta**d*tau**t*exp(-a*(delta-e)**2-b*(tau-g)**2)*(
                    t/tau-2*b*(tau-g))*(d/delta-2*a*(delta-e))
            Ct += n*tau**t*exp(-a*(delta-e)**2-b*(tau-g)**2) * \
                (t/tau-2*b*(tau-g)) * \
                (-2*a*delta**d + 4*a**2*delta**d*(delta-e)**2 -
                 4*d*a*delta**(d-1)*(delta-e) + d*(d-1)*delta**(d-2))
        ...
        for n, a, b, A, B_, C_, D, bt in zip(nr4, a4, b4, Ai, Bi, Ci, Di, bt4):
            ...
            C += n*(Delta**b*(2*Fd+delta*Fdd)+2*DeltaBd*(F+delta*Fd) +
                    DeltaBdd*delta*F)
            Bt += n*(Delta**b*(Ft+delta*Fdt)+delta*DeltaBd*Ft +
                     DeltaBt*(F+delta*Fd)+DeltaBdt*delta*F)
            Ct += n*(Delta**b*(2*Fdt+delta*Fddt) + DeltaBt*(2*Fd+delta*Fdd)
                     + 2*DeltaBdt*(F+delta*Fd) + 2*DeltaBd*(Ft+delta*Fdt)
                     + DeltaBddt*delta*F+DeltaBdd*delta*Ft)
